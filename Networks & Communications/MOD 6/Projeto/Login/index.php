...
<body class="min-vh-100">
    ...
    <?php
        if(isset($_GET['content'])) {
            ...
            else if ($_GET['content'] == 'EmailSentSuccessfully') {
                echo "<div class=\"container-fluid\" style=\"display:flex; justify-content:center;\">
                    <div class=\"alert alert-dismissible alert-costum fade show d-flex align-items-center mb-2 mt-0\" role=\"alert\">
                        <div class=\"text-light\">Foi-lhe enviado um email para repor a password</div>
                        <button type=\"button\" class=\"btn-close\" data-bs-dismiss=\"alert\" aria-label=\"Close\"></button>
                    </div>
                </div>";
            }
            else if ($_GET['content'] == 'EmailSentUnsuccessfully') {
                echo "<div class=\"container-fluid\" style=\"display:flex; justify-content:center;\">
                    <div class=\"alert alert-dismissible alert-costum fade show d-flex align-items-center mb-2 mt-0\" role=\"alert\">
                        <div class=\"text-light\">Ocorreu um erro ao enviar o email. Tente mais tarde</div>
                        <button type=\"button\" class=\"btn-close\" data-bs-dismiss=\"alert\" aria-label=\"Close\"></button>
                    </div>
                </div>";
            }
            else if ($_GET['content'] == 'PasswordChangedSuccessfully') {
                echo "<div class=\"container-fluid\" style=\"display:flex; justify-content:center;\">
                    <div class=\"alert alert-dismissible alert-costum fade show d-flex align-items-center mb-2 mt-0\" role=\"alert\">
                        <div class=\"text-light\">A password foi alterada com sucesso</div>
                        <button type=\"button\" class=\"btn-close\" data-bs-dismiss=\"alert\" aria-label=\"Close\"></button>
                    </div>
                </div>";
            }
    ...
    <div class="form-signin w-100 m-auto">
        <form class="rounded-3 needs-validation" action="logar.php" method="POST" novalidate>
            <div class="container-cargo">    
                <h1 class="h3 mb-3 fw-normal mt-2">Sign In</h1>
                <div class="dropdown dropend">
                    <button class="btn btn-light btn-select dropdown-toggle" type="button" id="cargoButton" data-bs-toggle="dropdown" aria-expanded="false">
                        Admin
                    </button>
                    <input type="hidden" name="cargoInput" id="cargoInput" value="admin">
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <li><a class="dropdown-item" onclick="mudarOpcao('Gestor ')">Gestor</a></li>
                        <li><a class="dropdown-item" onclick="mudarOpcao('Admin ')">Administrador</a></li>
                    </ul>
                </div>
            </div>
    ...
    <script>
        function mudarOpcao(opcao) {
            $('#cargoButton').text(opcao);
            $('#cargoInput').val(opcao.toLowerCase());
        }
    </script>
