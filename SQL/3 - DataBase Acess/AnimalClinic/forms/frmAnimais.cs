...
namespace AnimalClinic.forms
{
    public partial class frmAnimais : Form
    {
        ...
        {
            LoadTema();
            dgvAnimais.DataSource = bd.executeReturnDataQuery("SELECT * FROM Animais");
        }

        private void frmAnimais_VisibleChanged(object sender, EventArgs e)
        {
            if (Visible == true) {
                frmAnimais_Load(sender, e);
            }
        }
        ...
        private void btnRemover_Click(object sender, EventArgs e)
        {
            if (dgvAnimais.Rows.Count > 0)
            {
                DataRowView animalRowView = dgvAnimais.CurrentRow.DataBoundItem as DataRowView; // Obtem os dados da linha atual

                DataRow animal = animalRowView.Row;
                string tipo = animal["tipo"].ToString();
                string nome = animal["nome"].ToString();

                string msg = "Deseja remover o " + tipo + " " + nome + "?";
                DialogResult resultado = MessageBox.Show(msg, Clinica.appname, MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (resultado == DialogResult.Yes)
                {
                    int animalId = Convert.ToInt32(animal["id"]);
                    bd.executeQuery("DELETE FROM Animais WHERE id = " + animalId);
                    frmAnimais_Load(sender, e);
                }
            }
            else
        ...
        private void btnConsulta_Click(object sender, EventArgs e)
        {
            if (dgvAnimais.Rows.Count > 0)
            {
                ...
                DataRow animal = animalRowView.Row;
                frmEditAnimais form = new frmEditAnimais(false, Convert.ToInt32(animal["id"]));
                form.ShowDialog();
                frmAnimais_Load(sender, e);
                form.Dispose();
            }
